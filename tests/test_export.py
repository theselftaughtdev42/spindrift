...
def search_url_id(client, url):
    ...
    for choice in settings.split('<div class="search-url">')[1:]:
        if f'<span class="url">{url}</span>' in choice:
            return int(re.search(r'value="(\d+)"', choice)[1])
    raise AssertionError(f"{url} is not a saved search URL")


def test_an_export_holds_every_game_with_its_availabilities_intent_and_status(client):
...
def test_an_export_holds_every_search_url_and_which_one_is_active(client):
    client.post("/settings/urls", data={"url": SEARCH_URL})
    client.post("/settings/urls", data={"url": OTHER_SEARCH_URL})
    client.post(
        "/settings/active", data={"active": search_url_id(client, OTHER_SEARCH_URL)}
    )
...
def test_a_snapshot_declares_which_format_it_is(client):
    assert exported(client)["spindrift"] == FORMAT_VERSION


def test_exports_of_unchanged_data_are_identical(client):
    add_game(client, "Hades", ["Steam"])
    client.post("/settings/urls", data={"url": SEARCH_URL})

    assert (
        client.get("/settings/export").get_data()
        == client.get("/settings/export").get_data()
    )


def test_an_export_is_named_with_todays_date(client):
...
def test_an_export_is_sent_as_json(client):
    assert (
        client.get("/settings/export").headers["Content-Type"]
        == "application/json; charset=utf-8"
    )


def test_an_empty_deployment_exports_an_empty_snapshot(client):
