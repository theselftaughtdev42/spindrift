...
def save_search_url(client, url):
...
def test_saving_a_search_url_says_which_host_was_added(client):
    response = client.post("/settings/urls", data={"url": SEARCH_URL}, headers=HTMX)

    assert "example.com added." in words(response)


def test_a_search_url_without_the_placeholder_says_where_the_games_name_goes(client):
...
def test_a_refused_search_url_is_left_in_the_field(client):
    response = client.post(
        "/settings/urls", data={"url": "https://example.com/search"}, headers=HTMX
    )
...
def test_a_blank_search_url_saves_nothing(client):
...
def test_a_search_url_already_saved_is_refused_whatever_its_capitalisation(client):
...
def test_deleting_a_search_url_says_it_is_gone(client):
...
def test_making_a_search_url_active_says_which_host_searches_will_use(client):
...
def test_making_a_second_search_url_active_deactivates_the_first(client):
...
def test_turning_search_off_says_it_is_off(client):
    url_id = save_search_url(client, SEARCH_URL)
...
def test_the_catalogue_offers_no_search_control_while_no_search_url_is_active(client):
    add_game(client, "Hollow Knight")

    assert 'class="search"' not in client.get("/").get_data(as_text=True)


def test_the_catalogue_offers_a_search_control_once_a_search_url_is_active(client):
    add_game(client, "Hollow Knight")
    url_id = save_search_url(client, SEARCH_URL)
...
def test_the_search_control_carries_the_games_name_url_encoded(client):
    add_game(client, "Hollow Knight")
    url_id = save_search_url(client, SEARCH_URL)
    client.post("/settings/active", data={"active": url_id}, headers=HTMX)
...
def test_a_refused_search_url_reopens_the_settings_page_at_the_search_group(client):
    response = client.post("/settings/urls", data={"url": "https://example.com/search"})

    assert reopened_groups(response) == ["Search URL"]


def test_a_refused_import_reopens_the_settings_page_at_the_snapshot_group(client):
    response = import_snapshot(client, snapshot_data(), confirm=False)

    assert reopened_groups(response) == ["Snapshot"]


def test_a_refused_reset_reopens_the_settings_page_at_the_reset_group(client):
    response = client.post("/settings/reset", data={})

    assert reopened_groups(response) == ["Reset"]


def test_without_javascript_a_saved_search_url_returns_to_the_settings_page(client):
...
def test_with_htmx_a_saved_search_url_answers_with_the_search_group_itself(client):
    response = client.post("/settings/urls", data={"url": SEARCH_URL}, headers=HTMX)
